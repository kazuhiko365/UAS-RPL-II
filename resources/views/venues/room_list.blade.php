...
<div class="relative bg-gray-900 text-white overflow-hidden rounded-b-3xl shadow-xl -mt-6 mb-10">
    ...
    <div class="relative max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row items-end justify-between gap-6">
        <div>
            <a href="{{ route('cari.mabar') }}" class="inline-flex items-center text-emerald-300 hover:text-white text-sm font-bold mb-4 transition gap-1">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                Kembali ke Peta
            </a>
            <h1 class="text-4xl md:text-5xl font-extrabold tracking-tight mb-2">{{ $venue->name }}</h1>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="group bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col">
            ...
            <div class="h-28 bg-gradient-to-r from-slate-800 to-slate-900 relative p-5">
                ...
                <div class="absolute -bottom-6 left-6">
                    <img src="https://ui-avatars.com/api/?name={{ $room->host->name }}&background=10b981&color=fff" class="w-14 h-14 rounded-2xl border-4 border-white shadow-md group-hover:scale-110 transition duration-300">
                </div>
            </div>

            <div class="p-6 pt-10 flex-grow flex flex-col">
                <div class="mb-4">
                    <p class="text-xs text-gray-400 font-bold uppercase mb-1">Host: {{ $room->host->name }}</p>
                    <h3 class="text-xl font-bold text-gray-900 leading-tight group-hover:text-emerald-600 transition">{{ $room->title }}</h3>
                </div>

                <div class="grid grid-cols-2 gap-3 mb-6">
                    <div class="bg-gray-50 p-3 rounded-xl border border-gray-100">
                        <span class="block text-xs text-gray-400 mb-1">Jadwal</span>
                        <span class="block text-sm font-bold text-gray-800">{{ $room->start_datetime->format('d M') }}</span>
                        <span class="block text-xs text-emerald-600 font-bold">{{ $room->start_datetime->format('H:i') }} WIB</span>
                    </div>
                    <div class="bg-gray-50 p-3 rounded-xl border border-gray-100">
                        <span class="block text-xs text-gray-400 mb-1">Biaya</span>
                        <span class="block text-sm font-bold text-gray-800">Rp {{ number_format($room->cost_per_person/1000, 0) }}rb</span>
                ...
                <div class="mb-6">
                    <div class="flex justify-between text-xs mb-1 font-bold">
                        <span class="{{ $room->participants->count() >= $room->max_participants ? 'text-red-500' : 'text-gray-500' }}">
                            {{ $room->participants->count() }} Terisi
                        </span>
                        <span class="text-gray-400">Max {{ $room->max_participants }}</span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
                        <div class="h-full rounded-full transition-all duration-500 {{ $room->participants->count() >= $room->max_participants ? 'bg-red-500' : 'bg-emerald-500' }}" 
                             style="width: {{ ($room->participants->count() / $room->max_participants) * 100 }}%">
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-auto">
                    ...
                    @else
                        <a href="{{ route('rooms.show', $room) }}" class="flex items-center justify-center gap-2 w-full bg-gray-900 text-white hover:bg-emerald-600 font-bold py-3 rounded-xl transition shadow-lg transform active:scale-95">
                            Gabung Sekarang
        ...
        <div class="col-span-full flex flex-col items-center justify-center py-16 bg-white rounded-3xl border-2 border-dashed border-gray-200 text-center">
            ...
            <a href="{{ route('rooms.create', ['venue_id' => $venue->id]) }}" class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-6 rounded-full transition shadow-lg hover:shadow-emerald-200">
