                    ...
                    <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex flex-col sm:flex-row items-center gap-6 mb-8">
                            <div class="relative group">
                                <div class="w-28 h-28 rounded-full overflow-hidden border-4 border-emerald-100 shadow-md">
                                    @if(Auth::user()->avatar)
                                        <img id="preview-avatar" src="{{ asset('storage/' . Auth::user()->avatar) }}" class="w-full h-full object-cover">
                                    @else
                                        <img id="preview-avatar" src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=10b981&color=fff&size=128" class="w-full h-full object-cover">
                                    @endif
                                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name', Auth::user()->name) }}" class="w-full border-gray-300 rounded-xl focus:ring-emerald-500 focus:border-emerald-500 px-4 py-2.5 transition">
                                @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>
                            
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">Nomor WhatsApp</label>
                                <input type="text" name="phone" value="{{ old('phone', Auth::user()->phone) }}" class="w-full border-gray-300 rounded-xl focus:ring-emerald-500 focus:border-emerald-500 px-4 py-2.5 transition" placeholder="08...">
                        ...
                        <div class="mb-8">
                            ...
                            <textarea name="bio" rows="2" class="w-full border-gray-300 rounded-xl focus:ring-emerald-500 focus:border-emerald-500 px-4 py-2.5 transition" placeholder="Contoh: Kiper Futsal, Main Santai, Cari Keringat">{{ old('bio', Auth::user()->bio) }}</textarea>
        ...
        <div class="lg:col-span-1 space-y-8">
            ...
            <div class="bg-blue-50 rounded-2xl p-6 border border-blue-100 shadow-sm">
                ...
                <div class="space-y-2 text-sm text-blue-700">
                    <p>Status: <span class="font-bold uppercase bg-blue-200 px-2 py-0.5 rounded text-xs">{{ Auth::user()->role }}</span></p>
                    <p>Email: <span class="font-medium">{{ Auth::user()->email }}</span></p>
                    <p>Bergabung: <span class="font-medium">{{ Auth::user()->created_at->format('d M Y') }}</span></p>
                </div>
            </div>
