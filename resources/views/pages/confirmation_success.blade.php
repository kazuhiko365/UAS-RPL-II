        ...
        <div class="p-8">
            ...
            <div class="space-y-4 border-t border-gray-100 pt-6">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-bold text-gray-800">{{ $participant->user->name }}</span>
                </div>
                
                <div class="flex justify-between items-center">
                    <span class="text-gray-500 text-sm">🏆 Room/Judul</span>
                    <span class="font-bold text-gray-800 text-right truncate w-40">{{ $participant->room->title }}</span>
                </div>

                <div class="flex justify-between items-center">
                    <span class="text-gray-500 text-sm">⚽ Cabang Olahraga</span>
                    <span class="font-bold text-gray-800">{{ $participant->room->sport->name }}</span>
                </div>

                <div class="flex justify-between items-center">
                    <span class="text-gray-500 text-sm">📅 Jadwal Main</span>
                    <span class="font-bold text-gray-800 text-right">
                        {{ $participant->room->start_datetime->format('d M Y') }}<br>
                        <span class="text-xs text-gray-400">Pukul {{ $participant->room->start_datetime->format('H:i') }} WIB</span>
                    </span>
                </div>

                <div class="flex justify-between items-center">
                    <span class="text-gray-500 text-sm">👑 Host Room</span>
                    <span class="font-bold text-gray-800">{{ $participant->room->host->name }}</span>
                </div>
            </div>

            <div class="mt-8 space-y-3">
                <a href="{{ route('login') }}" class="block w-full bg-gray-900 text-white text-center py-3 rounded-xl font-bold shadow-lg hover:bg-black transition transform active:scale-95">
                    Masuk ke Aplikasi
                </a>
            </div>
        </div>
