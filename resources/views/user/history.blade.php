...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <a href="{{ route('dashboard') }}" class="flex items-center gap-2 text-sm font-bold text-gray-600 hover:text-emerald-600 transition">
    ...
    @if($histories->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden opacity-90 hover:opacity-100 transition shadow-sm hover:shadow-md">
                ...
                <div class="p-5">
                    <h3 class="font-bold text-gray-800 text-lg mb-1 line-clamp-1">{{ $room->title }}</h3>
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                        <div class="flex items-center gap-2">
                            <img src="{{ $room->host->avatar ? asset('storage/' . $room->host->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode($room->host->name) . '&background=random' }}" class="w-8 h-8 rounded-full border border-gray-200">
                            <div class="text-xs">
                                <span class="block text-gray-400">Host</span>
                                <span class="font-bold text-gray-700">{{ $room->host_id == auth()->id() ? 'Anda' : Str::limit($room->host->name, 10) }}</span>
                            </div>
                        </div>
                        
                        <a href="{{ route('rooms.show', $room) }}" class="text-xs font-bold text-gray-500 hover:text-emerald-600 border border-gray-300 px-3 py-1.5 rounded-lg hover:border-emerald-500 transition">
                            Lihat Detail
                        </a>
                    </div>
                </div>
