...
<div class="relative bg-emerald-900 overflow-hidden mb-10">
    ...
    <div class="relative max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8 text-center md:text-left">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
            <a href="{{ route('cari.mabar') }}" class="px-8 py-3 bg-white text-emerald-800 font-bold rounded-full shadow-lg hover:bg-emerald-50 transition transform hover:-translate-y-1 flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                Cari Room
            </a>

            @auth
            <a href="{{ route('dashboard') }}" class="px-8 py-3 bg-emerald-700 border border-emerald-600 text-white font-bold rounded-full shadow-lg hover:bg-emerald-600 transition transform hover:-translate-y-1 flex items-center justify-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
        ...
        <div class="group bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col h-full">
            ...
            <div class="h-32 bg-gradient-to-br from-emerald-500 to-teal-700 relative p-5 flex flex-col justify-between">
                <div class="flex justify-between items-start">
                    ...
                    <span class="bg-black/30 backdrop-blur text-white text-[10px] font-bold px-2 py-1 rounded-lg flex items-center gap-1">
                        ...
                        {{ number_format($room->distance_km, 1) }} km
                    </span>
                    @endif
                </div>
            </div>
            ...
            <div class="p-6 pt-8 relative flex-grow flex flex-col">
                ...
                <div class="mb-1">
                    <p class="text-xs text-gray-400 font-bold uppercase mb-1 ml-16 pl-2">Host: {{ Str::limit($room->host->name, 15) }}</p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 mb-5">
                    <div class="bg-gray-50 p-2.5 rounded-xl border border-gray-100 flex flex-col justify-center">
                        ...
                        <div class="text-xs text-emerald-600 font-bold">{{ $room->start_datetime->format('H:i') }} WIB</div>
                    </div>
                    <div class="bg-gray-50 p-2.5 rounded-xl border border-gray-100 flex flex-col justify-center">
                        <span class="text-[10px] text-gray-400 uppercase font-bold">Biaya</span>
                        <div class="text-sm font-bold text-gray-800">Rp {{ number_format($room->cost_per_person/1000, 0) }}rb</div>
                    ...
                    <div class="flex -space-x-2 overflow-hidden items-center">
                        @foreach($room->participants->take(3) as $p)
                            <img class="inline-block h-6 w-6 rounded-full ring-2 ring-white object-cover" 
                                 src="{{ $p->user->avatar ? asset('storage/' . $p->user->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode($p->user->name) . '&background=random' }}" 
                                 title="{{ $p->user->name }}">
                        @endforeach
