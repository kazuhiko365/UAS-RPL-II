...
@section('content')
                    ...
                    <div class="mb-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                            ...
                            <div>
                                ...
                                @if(isset($selectedVenueId) || request('venue_id'))
                                    <div
                                ...
                                <div class="relative">
                                    <select id="venue_id" name="venue_id"
                                        class="appearance-none w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block px-4 py-3.5 transition cursor-pointer font-medium">
                                        <option value="" disabled {{ (!isset($selectedVenueId) && !request('venue_id')) ? 'selected' : '' }} class="text-gray-400">Pilih Lokasi</option>
                                        @foreach($venues as $venue)
                                            <option value="{{ $venue->id }}" {{ (old('venue_id', $selectedVenueId ?? request('venue_id')) == $venue->id) ? 'selected' : '' }}>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="3"
                                    ...
                                    placeholder="Tulis info penting: bawa raket sendiri, patungan di lokasi, level pemula welcome, dll.">{{ old('description', request('description') ?? '') }}</textarea>
                            </div>
                        </div>
                    </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" step="1000" id="cost_per_person" name="cost_per_person"
                                        value="{{ old('cost_per_person', request('cost_per_person', 0) ?? '') }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const currentSelectedVenue = "{{ old('venue_id', $selectedVenueId ?? request('venue_id')) }}";
            ...
            function fetchVenues(sportId) {
                ...
                fetch(`{{ route('venues.filterBySport') }}?sport_id=${sportId}`)
