...
<div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start mb-8 gap-6">
        <div class="flex-1">
            ...
            <div class="flex items-center gap-2 text-gray-500 text-sm">
                ...
                <span class="font-medium">{{ $room->venue->name }}</span>
                <span class="text-gray-300 mx-1">|</span>
                <span>{{ $room->venue->city }}</span>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-start gap-4">
            ...
            <div>
                ...
                <p class="font-bold text-gray-900 text-2xl">Rp {{ number_format($room->cost_per_person, 0, ',', '.') }}</p>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-center">
            <div class="flex justify-between items-end mb-2">
                ...
                <p class="text-sm font-bold text-gray-900">{{ $room->participants->count() }} / {{ $room->max_participants }}</p>
            </div>
            <div class="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                <div class="bg-gradient-to-r from-emerald-400 to-green-600 h-full rounded-full transition-all duration-1000 ease-out" style="width: {{ ($room->participants->count() / $room->max_participants) * 100 }}%"></div>
            </div>
            <p class="text-right text-[10px] text-emerald-600 font-bold mt-2">
                {{ $room->max_participants - $room->participants->count() }} tempat tersisa
            </p>
        </div>
    </div>
    ...
    {{-- LOGIKA TOMBOL GABUNG / STATUS (UPDATE PENTING) --}}
    @php
    ...
    {{-- 2. Tampilan Room Closed (Waktu Habis) --}}
    @elseif($isStarted)
    ...
    {{-- 3. Tampilan Sudah Join (Status Peserta) --}}
    @elseif($myStatus)
        <div class="mb-12 p-6 rounded-2xl border-2 {{ $myStatus == 'confirmed' ? 'bg-emerald-50 border-emerald-200' : 'bg-yellow-50 border-yellow-200' }}">
            <div class="flex items-center justify-center gap-4">
                <div class="p-3 rounded-full {{ $myStatus == 'confirmed' ? 'bg-emerald-100 text-emerald-600' : 'bg-yellow-100 text-yellow-600' }}">
                ...
                <div>
                    <h3 class="font-bold text-xl {{ $myStatus == 'confirmed' ? 'text-emerald-800' : 'text-yellow-800' }}">
                        {{ $myStatus == 'confirmed' ? 'Anda Terdaftar!' : 'Menunggu Konfirmasi' }}
                    </h3>
                    <p class="text-sm {{ $myStatus == 'confirmed' ? 'text-emerald-600' : 'text-yellow-600' }}">
                        {{ $myStatus == 'confirmed' ? 'Sampai jumpa di lapangan!' : 'Host sedang meninjau request Anda.' }}
                    </p>
                </div>
            </div>
        </div>
    ...
    @elseif($isHost)
        <div class="mb-12">
            <a href="{{ route('rooms.edit', $room) }}" class="block w-full bg-gray-800 hover:bg-black text-white font-bold py-4 rounded-xl text-center transition shadow-lg flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                Edit Detail Room
            </a>
        </div>

    {{-- 5. Tampilan Join untuk User Biasa --}}
    @else
        <form action="{{ route('rooms.join', $room) }}" method="POST" class="mb-12">
            @csrf
            @if($isFull)
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <span class="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-xs font-bold">{{ $room->participants->count() }} Orang</span>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div class="p-5 border-b border-gray-50 flex items-center justify-between last:border-0 hover:bg-gray-50 transition">
            ...
            <div class="flex items-center gap-3">
                <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border {{ $participant->status == 'confirmed' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-yellow-50 text-yellow-600 border-yellow-100' }}">
                    {{ ucfirst($participant->status) }}
                </span>

                {{-- Tombol Terima (Hanya untuk Host) --}}
                @if(auth()->id() == $room->host_id && $participant->status == 'requested')
